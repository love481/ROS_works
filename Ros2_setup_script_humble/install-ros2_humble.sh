...
Var=$(lsb_release -r)
echo "$Var"
ubuntu_version=$(cut -f2 <<< "$Var")
echo ""
echo "[Note] OS version  >>> $ubuntu_version"
...
echo "[Make the colcon workspace and install lidar package and other dependencies]"
cd $HOME
